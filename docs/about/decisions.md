...
| D23 | Add a **terminology layer**: a hand-written concept scheme (`TERMS.yaml`: SKOS-style preferred/alternative labels, `broader` with polyhierarchy, `close` matches, facets as classes) plus a small **ontology** (9 typed relations with domain and range), checked and **grounded** by `build_terms.py` (every non-grouping concept must occur in a manual) and counted per manual into a committed `terms.json` of numbers only | the gear's manuals name the same ideas differently; a flat tag list cannot say "synonym" vs "kind of" vs "similar", or which kinds of concept a relation may join; publishing counts, not text, keeps the manual text private | the scheme is one modeller's judgement and counts are string matches, not meanings; process and pitfalls in [How the terminology was built](terminology.md) | owner asked for it; assistant designed the scheme; **not yet reviewed by the owner** |
